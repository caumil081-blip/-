...
NEW_ABS = ('군 내부 이메일에는 지시·보고 요청·자료 제출 같은 행정 업무가 마감과 함께 흩어져 있어 수신자가 일일이 해석해 우선순위를 정해야 한다. 본 논문은 데이터를 외부로 보내지 않는 로컬 대규모 언어모델(LLM)로 업무 정보를 구조화해 추출하고, 기한·발신자 권한·업무유형을 가중 결합한 규칙 엔진으로 우선순위를 산정하는 시스템을 제안한다. 합성 군 이메일 1,150건 평가에서 LLM(qwen2.5:7b)은 업무 탐지 F1 0.979, 업무유형 macro-F1 0.902를 기록하였고, 우선순위 엔진은 가중치 변화에도 안정적인 3등급 순위를 제시하였다. 계층화 분석은 암시적 마감의 재현율 한계가 후단 규칙 기반 날짜 정규화에 있음을 밝힌다.')
